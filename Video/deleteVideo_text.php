...
if(!is_numeric($_POST['textID']) || !is_numeric($_POST['signLanguage']) || !is_numeric($_POST['alt']))
{
	exit();
}
...
$result = $s3->deleteObject(array(
    ...
    'Key'    => 'English/Videos/' . $_POST['textID'] . '-' . $_POST['alt'] . '-' . $_POST['signLanguage'] . '_text.mp4'
));

$connect->exec("DELETE FROM TextVideos WHERE nTextID = :id AND nAltID = :alt AND nSignLanguage = :signLanguage",
[
	'id' => $_POST['textID'],
...
[
	'name' => $_POST['textID'] . '-' . $_POST['alt'] . '-' . $_POST['signLanguage'] . '_text.mp4'
]);
...
$POST_DATA =
[
	'video' => $_POST['wordID'] . '-' . $_POST['alt'] . '-' . $_POST['signLanguage'] . '_text'
];
...
include_once __DIR__ . '/RetranslateVideoText.php';
exit();