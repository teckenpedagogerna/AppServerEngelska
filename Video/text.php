...
header("Location: " . "https://" . $_SERVER['HTTP_HOST'] . '/AppServer/Engelska/Video/text2.php');
exit();
...
$texts = $connect->query("SELECT * FROM Texts WHERE nLanguageID = :languageID ORDER BY sText",
[
    "languageID" => $_GET['lID']
]);
...
?>
            <tr>
    			<th><?php echo (1 + $i); ?></th>
    			<th><?php echo $texts[$i]['sText']; ?></th>
    			<th><?php echo $texts[$i]['nUsage']; ?></th>
    			<th><button class="btn btn-primary" style="width: 100%;" onclick="Whereis('<?php echo $texts[$i]['sFoundAt']; ?>');">Visa</button></th>
    			<th>
    				<select id="videos_1_<?php echo $texts[$i]['nID']; ?>">
...
?>
					...
					<button class="btn btn-primary" onclick="UploadVideo(<?php echo "'" . htmlspecialchars($texts[$i]['sText']) . '\', ' . $texts[$i]['nID']; ?>, 1);">Ladda upp</button>
					<button class="btn btn-secondary" onclick="ShowVideo(<?php echo "'" . htmlspecialchars($texts[$i]['sText']) . '\', ' . $texts[$i]['nID']; ?>, 1)">Visa</button>
    			</th>
    			<th>
    				<select id="videos_2_<?php echo $texts[$i]['nID']; ?>">
                        <option value="none">None</option>
<?php
...
?>
					...
					<button class="btn btn-primary" onclick="UploadVideo(<?php echo "'" . htmlspecialchars($texts[$i]['sText']) . '\', ' . $texts[$i]['nID']; ?>, 2);">Ladda upp</button>
                    <button class="btn btn-secondary" onclick="ShowVideo(<?php echo "'" . htmlspecialchars($texts[$i]['sText']) . '\', ' . $texts[$i]['nID']; ?>, 2)">Visa</button>
    			</th>
    		</tr>
<?php
}
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body" id="modal_show_video_content">
                    ...
                    <input onchange="ChangeTranslate(this);" id="translation_video" type="text" class="form-control" style="width:70%; float: left;" placeholder="Översätt" aria-describedby="basic-addon1" value="">
