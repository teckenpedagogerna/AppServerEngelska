...
$words_swe = $connect->query("
	SELECT 
		t.nID, ws.nID AS wID,
		ws.sWord, ws.nUsage,
		t.sFoundAt, t.nEnglishID, t.nSwedishID
	FROM Translations AS t 
	LEFT JOIN Words AS ws ON (t.nSwedishID = ws.nID AND ws.nLanguageID = 1) 
	WHERE t.sFoundAt IS NOT NULL AND t.bIsText = 0 AND t.nSwedishID != 0
	ORDER BY ws.sWord",
	[
...
$words_eng = $connect->query("
	SELECT 
		t.nID, we.nID AS wID,
		we.sWord, we.nUsage,
		t.sFoundAt, t.nEnglishID, t.nSwedishID
	FROM Translations AS t 
	LEFT JOIN Words AS we ON (t.nEnglishID = we.nID AND we.nLanguageID = 2)
	WHERE t.sFoundAt IS NOT NULL AND t.bIsText = 0 AND t.nEnglishID != 0
	ORDER BY we.sWord",
	[
...
for ($e = 0; $e < count($words_eng); $e++)
{
	$words[$words_eng[$e]['nID']] = 
	[
		'nID' => $words_eng[$e]['nID'],
		'english' => $words_eng[$e]['sWord'],
		'swedish' => '',
		'usage' => $words_eng[$e]['nUsage'],
		'foundAt' => $words_eng[$e]['sFoundAt'],
		'usage' => $words_eng[$e]['nUsage']
	];
}

for ($s = 0; $s < count($words_swe); $s++)
{
	$words[$words_swe[$s]['nID']] = 
	[
		'nID' => isset($words[$words_swe[$s]['nID']]['nID']) ? $words[$words_swe[$s]['nID']]['nID'] : '',
		'english' => isset($words[$words_swe[$s]['nID']]['english']) ? $words[$words_swe[$s]['nID']]['english'] : '',
		'swedish' => $words_swe[$s]['sWord'],
		'foundAt' => isset($words[$words_swe[$s]['nID']]['foundAt']) ? $words[$words_swe[$s]['nID']]['foundAt'] : $words_swe[$s]['sFoundAt'],
		'usage' => isset($words[$words_swe[$s]['nID']]['usage']) ? $words[$words_swe[$s]['nID']]['usage'] : $words_swe[$s]['nUsage']
	];
}
...
<body>
	...
	<div class="card" style="margin: 24px;">
		<input id="search_input" onkeyup="Search(this);" type="text" class="form-control" placeholder="Sök" aria-describedby="basic-addon1">
		<div style="margin: 12px;">
			<input onclick="SearchRefresh();" type="checkbox" id="noVideo" name="noVideo"> <label for="noVideo"> No video</label><br>
			<?php echo 'Total ord: ' . count($words_eng) . ' st, översatta: ' . count($words_swe) . ' st'; ?>
		</div>
	</div>
	<div class="card">
...
?>
			<tr class="tablerow">
				<th><?php echo $word['nID']; ?></th>
				<th>
					<img src="../assets/img/flags/4x3/us.svg" height="40" style="border-radius:8px; opacity:0.5;" />
					<?php echo ucfirst($word['english']); ?>
				</th>
				<th>
					<img src="../assets/img/flags/4x3/se.svg" height="40" style="border-radius:8px; opacity:0.5;" />
					<?php echo ucfirst($word['swedish']); ?>
				</th>
				<th><?php echo $word['usage']; ?></th>
				<th><button class="btn btn-primary" style="width: 100%;" onclick="Whereis('<?php echo $word['foundAt']; ?>');">Visa</button></th>
				<th>
					<img src="../assets/img/flags/4x3/se.svg" height="40" style="border-radius:8px; opacity:0.5;" />
					<select id="videos_1_<?php echo $word['nID']; ?>">
...
?>
					...
					<button class="btn btn-primary" onclick="UploadVideo(<?php echo "'" . htmlspecialchars(ucfirst($word['english'])) . '\', ' . $word['nID']; ?>, 1);">Ladda upp</button>
					<button class="btn btn-secondary" onclick="ShowVideo(<?php echo "'" . htmlspecialchars(ucfirst($word['english'])) . '\', ' . $word['nID']; ?>, 1)">Visa</button>
				</th>
				<th>
					<img src="../assets/img/flags/4x3/us.svg" height="40" style="border-radius:8px; opacity:0.5;" />
					<select id="videos_2_<?php echo $word['nID']; ?>">
						<option value="none">None</option>
<?php
...
?>
					...
					<button class="btn btn-primary" onclick="UploadVideo(<?php echo "'" . htmlspecialchars(ucfirst($word['english'])) . '\', ' . $word['nID']; ?>, 2);">Ladda upp</button>
					<button class="btn btn-secondary" onclick="ShowVideo(<?php echo "'" . htmlspecialchars(ucfirst($word['english'])) . '\', ' . $word['nID']; ?>, 2)">Visa</button>
				</th>
			</tr>
<?php
}
